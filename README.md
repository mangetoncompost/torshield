...
<img src="assets/demo.gif" width="520" alt="TorShield demo" />
...
TorShield sits in your menubar. One click routes all your traffic through Tor — system-wide proxy, Firefox hardening, IPv6 disabled, MAC spoofed — then restores everything cleanly when you quit.
...
| Toggle | Default | Note |
|---|---|---|
| Firefox (proxy + WebRTC off) | ✓ | Patches `user.js` + live `prefs.js` |
| Firefox resistFingerprinting ⚠ | off | Disables WebGL/canvas — breaks some sites |
| MAC spoofing | ✓ | Randomizes `en0` hardware address |
| DNS leak fix (dnsmasq) | ✓ | Requires dnsmasq |
| Kill switch (pf firewall) | off | Blocks all non-Tor TCP outbound |
| Clear logs on start | ✓ | `log erase --all` + crash reporter purge |
| User-Agent spoofing | ✓ | Sends `Mozilla/5.0 (Windows NT 10.0…)` |
| Neutral language (en-US) | ✓ | Overrides `Accept-Language` header |
...
This tool is intended for **authorized security testing, privacy research, and personal use**. Routing traffic through Tor is legal in most jurisdictions. You are responsible for ensuring your use complies with applicable local laws and any terms of service.

Using this tool to access systems without authorization or to engage in illegal activity is prohibited.

## Contributing

Issues and PRs welcome. Zero warnings policy — run before submitting:
